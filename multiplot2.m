...
while 1
    ...
    switch xaxis
        case 'lin'
            cline = plot(fd.freq(1:ind-1) ./ (1000 * fnorm), dnorm + fd.avgDB(1:ind-1), 'DisplayName', dname);
        case 'log'
            cline = semilogx(fd.freq(1:ind-1) ./ (1000 * fnorm), dnorm + fd.avgDB(1:ind-1), 'DisplayName', dname);
        otherwise
